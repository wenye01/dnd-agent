...
describe('QuickActions', () => {
  ...
  describe('Save button feedback', () => {
    it('should display "Save" label initially', () => {
      render(<QuickActions />)

      expect(screen.getByTitle('Save')).toHaveTextContent('Save')
    })

    it('should add system message when Save is clicked', () => {
    ...
    it('should change label to "Saved!" after click', () => {
      vi.useFakeTimers()
      setupGameStateWithParty([createMockCharacter()])
      render(<QuickActions />)
    ...
    it('should revert label back to "Save" after timeout', () => {
      vi.useFakeTimers()
      setupGameStateWithParty([createMockCharacter()])
      render(<QuickActions />)
    ...
    it('should update metadata timestamp when metadata exists', () => {
      vi.useFakeTimers()
      ...
      const metadata = useGameStore.getState().gameState?.metadata
    ...
    it('should not crash when metadata is undefined', () => {
      // Set gameState without metadata by using updateGameState to clear it
      useGameStore.getState().setGameState({
        sessionId: 's1',
        phase: 'exploration',
