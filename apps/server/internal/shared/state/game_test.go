...
func TestActiveEffect(t *testing.T) {
	t.Run("active effect with conditions", func(t *testing.T) {
		effect := &ActiveEffect{
			ID:         "effect-1",
			Name:       "Poison",
			TargetID:   "char-1",
			Duration:   5,
			Conditions: []string{"poisoned", "weakened"},
		}

		if len(effect.Conditions) != 2 {
			t.Errorf("Conditions length = %d, want 2", len(effect.Conditions))
		}

		if effect.Conditions[0] != "poisoned" {
			t.Errorf("First condition = %s, want 'poisoned'", effect.Conditions[0])
		}
	})

	t.Run("active effect without conditions", func(t *testing.T) {
		effect := &ActiveEffect{
			ID:         "effect-2",
			Name:       "Bless",
			TargetID:   "char-2",
		...
		if len(effect.Conditions) != 0 {
			t.Errorf("Conditions should be empty, got %d elements", len(effect.Conditions))
		}
	})
}

func TestLocation(t *testing.T) {
	t.Run("location with position", func(t *testing.T) {
		loc := &Location{
			ID:          "loc-1",
			Name:        "Town Square",
		...
		if loc.Position.X != 50 {
			t.Errorf("Position X = %d, want 50", loc.Position.X)
		}
	})

	t.Run("location without position", func(t *testing.T) {
		loc := &Location{
			ID:          "loc-2",
		...
		if loc.Position != nil {
			t.Error("Position should be nil")
		}
	})
}

func TestGameMetadata(t *testing.T) {
	t.Run("metadata fields are set", func(t *testing.T) {
		...
		if metadata.CreatedAt != now {
			t.Errorf("CreatedAt = %d, want %d", metadata.CreatedAt, now)
		}

		if metadata.DM != "dm-user-123" {
			t.Errorf("DM = %s, want 'dm-user-123'", metadata.DM)
		}
	})
}
